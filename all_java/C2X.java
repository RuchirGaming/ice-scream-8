...
public final class C2X extends Drawable {
    ...
    public static void A05() {
        A0H = new byte[]{114, 124, 17, 41, 47, 40, 124, 62, 57, 124, 98, 97, 124, 108, 115, 84, 76, 91, 86, 83, 94, 26, 87, 91, 66, 26, 73, 82, 91, 94, 85, 77, 26, 73, 83, SignedBytes.MAX_POWER_OF_TWO, 95, 26, 8, 47, 55, 32, 45, 40, 37, 97, 51, 32, 37, 40, 52, 50, 97, 1, 38, 62, 41, 36, 33, 44, 104, 59, 32, 41, 44, 39, Utf8.REPLACEMENT_BYTE, 104, 59, 33, 50, 45, 104};
    }
    ...
    public C2X(Resources resources, ColorStateList colorStateList, float f2, float f3, float f4) {
        A07(colorStateList);
    ...
    public static float A00(float f2, float f3, boolean z) {
        if (z) {
            ...
            return (float) (d2 + ((1.0d - A0J) * ((double) f3)));
        }
        return f2;
    }
    ...
    public static float A01(float f2, float f3, boolean z) {
        if (z) {
            return (float) (((double) (1.5f * f2)) + ((1.0d - A0J) * ((double) f3)));
        }
        return 1.5f * f2;
    }
    ...
    private void A04() {
        ...
        this.A08.close();
        float f4 = this.A00;
        float f5 = f4 / (this.A03 + f4);
        Paint paint = this.A05;
        float f6 = this.A03 + this.A00;
        int i2 = this.A0E;
        paint.setShader(new RadialGradient(0.0f, 0.0f, f6, new int[]{i2, i2, this.A0D}, new float[]{0.0f, f5, 1.0f}, Shader.TileMode.CLAMP));
        Paint paint2 = this.A06;
        float f7 = this.A00;
        float startRatio = this.A03;
        float f8 = (-f7) + startRatio;
        float f9 = (-f7) - startRatio;
        int i3 = this.A0E;
        paint2.setShader(new LinearGradient(0.0f, f8, 0.0f, f9, new int[]{i3, i3, this.A0D}, new float[]{0.0f, 0.5f, 1.0f}, Shader.TileMode.CLAMP));
    ...
    private void A06(float f2, float f3) {
        ...
        if (f2 < 0.0f) {
            throw new IllegalArgumentException(A03(53, 20, 13) + f2 + strA03);
        }
        if (A0I[1].length() == 0) {
            ...
            if (f3 >= 0.0f) {
                ...
                throw new IllegalArgumentException(A03(14, 24, 127) + f3 + strA03);
            }
        }
        throw new RuntimeException();
    }
    ...
    private void A08(Canvas canvas) {
        ...
        float f5 = this.A0F.left + inset;
        float edgeShadowTop5 = this.A0F.top;
        canvas.translate(f5, edgeShadowTop5 + inset);
        ...
        if (z) {
            ...
            canvas.drawRect(0.0f, f3, fWidth, -edgeShadowTop7, this.A06);
        }
        canvas.restoreToCount(iSave);
        int iSave2 = canvas.save();
        float f6 = this.A0F.right - inset;
        float edgeShadowTop8 = this.A0F.bottom;
        canvas.translate(f6, edgeShadowTop8 - inset);
        ...
        if (z) {
            ...
            float f7 = -edgeShadowTop10;
            float edgeShadowTop11 = this.A03;
            canvas.drawRect(0.0f, f3, fWidth2, f7 + edgeShadowTop11, this.A06);
        }
        canvas.restoreToCount(iSave2);
        int iSave3 = canvas.save();
        float f8 = this.A0F.left + inset;
        float edgeShadowTop12 = this.A0F.bottom;
        canvas.translate(f8, edgeShadowTop12 - inset);
        if (A0I[2].length() != 3) {
            A0I[2] = "QAUm";
            ...
            if (z2) {
                ...
                canvas.drawRect(0.0f, f3, fHeight, -edgeShadowTop14, this.A06);
            }
            canvas.restoreToCount(iSave3);
            int iSave4 = canvas.save();
            float f9 = this.A0F.right - inset;
            float f10 = this.A0F.top;
            if (A0I[2].length() != 3) {
                A0I[2] = "4hs9knzgsM3vbS2";
                canvas.translate(f9, f10 + inset);
                canvas.rotate(90.0f);
                canvas.drawPath(this.A08, this.A05);
                if (z2) {
    ...
    private void A09(Rect rect) {
        ...
        float f4 = f3 + verticalOffset;
        float f5 = rect.top + f2;
        float f6 = rect.right;
        float verticalOffset2 = this.A01;
        float f7 = f6 - verticalOffset2;
        float verticalOffset3 = rect.bottom;
        rectF.set(f4, f5, f7, verticalOffset3 - f2);
        A04();
    }
    ...
    public final float A0C() {
        float f2 = this.A01;
        float fMax = Math.max(f2, this.A00 + this.A0C + ((f2 * 1.5f) / 2.0f)) * 2.0f;
        float f3 = this.A01 * 1.5f;
        float content = this.A0C;
        return ((f3 + content) * 2.0f) + fMax;
    }

    public final float A0D() {
        float f2 = this.A01;
        float fMax = Math.max(f2, this.A00 + this.A0C + (f2 / 2.0f)) * 2.0f;
        float f3 = this.A01;
        float content = this.A0C;
        return ((f3 + content) * 2.0f) + fMax;
    }
