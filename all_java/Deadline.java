...
public final class Deadline implements Comparable<Deadline> {
    ...
    public static Deadline after(long j2, TimeUnit timeUnit) {
        return after(j2, timeUnit, SYSTEM_TICKER);
    }
    ...
    private Deadline(Ticker ticker, long j2, long j3, boolean z) {
        this.ticker = ticker;
        long jMin = Math.min(MAX_OFFSET, Math.max(MIN_OFFSET, j3));
        this.deadlineNanos = j2 + jMin;
    ...
    public String toString() {
        ...
        long j3 = jAbs / j2;
        long jAbs2 = Math.abs(jTimeRemaining) % j2;
    ...
    public int compareTo(Deadline deadline) {
        checkTicker(deadline);
        long j2 = this.deadlineNanos - deadline.deadlineNanos;
        if (j2 < 0) {
            return -1;
        }
        return j2 > 0 ? 1 : 0;
